...
export default function FinalDashboard({ patientInfo, timelineData, assignedSubject, pipelineOutputs, onReset }) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const fileName = `MedMatrix_Clinical_Report_${patientInfo.name.replace(/\s+/g, '_')}.pdf`;
      pdf.save(fileName);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-[var(--indigo)] text-white text-xs font-bold shadow-md hover:bg-blue-600 transition-colors disabled:opacity-50"
          >
      ...
      {/* Printable / Canvas Capture Container */}
      <div ref={reportRef} id="pdf-clinical-report" className="space-y-6 bg-white p-2">
        {/* Printable Official Header (Visible on PDF/Print) */}
        <div className="hidden print-header p-4 bg-slate-900 text-white rounded-2xl flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-white font-bold">
              <Activity className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-extrabold text-base tracking-tight">Med Matrix AI — N-of-1 Clinical Decision Report</h2>
              <p className="text-[10px] text-blue-200 font-mono">Personalized Precision Medicine Platform</p>
            </div>
          </div>
          <div className="text-right text-[10px] font-mono text-slate-300">
            <div>Report ID: MM-2026-{(patientInfo.name.length * 137) % 9000 + 1000}</div>
        ...
        <div className="content-card border-2 border-[var(--indigo)]/20 shadow-lg relative overflow-hidden bg-white">
          <div className="relative z-10">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-50 to-white border border-emerald-200 space-y-4 mb-6">
